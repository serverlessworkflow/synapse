...
namespace Synapse.Application.Services
{
        ...
        : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            this.FileSystemWatcher.Created += this.OnFileCreatedAsync;
            foreach (var file in directory.GetFiles("*.*", SearchOption.AllDirectories)
                .Where(f => f.Extension.ToLower() == ".json" || f.Extension.ToLower() == ".yml" || f.Extension.ToLower() == ".yaml"))
            {
                await this.ReadAndCreateWorkflowAsync(file.FullName);
            }
        }
        ...
        protected virtual async void OnFileCreatedAsync(object sender, FileSystemEventArgs e)
        {
            ...
            await this.ReadAndCreateWorkflowAsync(e.FullPath);
        }
        ...
        protected virtual async Task ReadAndCreateWorkflowAsync(string filePath)
        {
            try
            {
                ...
                await scope.ServiceProvider.GetRequiredService<IMediator>().ExecuteAndUnwrapAsync(new V1CreateWorkflowCommand(definition, true), this.CancellationTokenSource.Token);
            }
            catch (Exception ex)
            {
