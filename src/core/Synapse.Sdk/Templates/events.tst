...
${
    ...
    Template(Settings settings)
    {
        ...
        settings.OutputFilenameFactory = (file) => GetOutputFilename("event", file);
    }
    ...
    string GetOutputFilename(string templateType, File file)
    {
            ...
            && c.Name.EndsWith("DomainEvent"))
        {
            ...
            string directoryName = $"Events\\{directory.Parent.Name}\\{directory.Name}\\Generated";
    ...
    string NamespaceDeclaration(Class c)
    {
      ...
      output.AppendLine($"namespace Synapse.Sdk.Events.{new System.IO.FileInfo(((Typewriter.CodeModel.File)c.Parent).FullName).Directory.Parent.Name}");
        output.AppendLine("{");
...
}$Classes(Synapse.Domain.Events.*)[$NamespaceDeclaration
