    ...
    where TResource : Resource, new()
{
    ...
    EquatableList<TResource>? _resources;
    ...
    {
        this._definition = await this.ApiClient.ManageNamespaced<TResource>().GetDefinitionAsync().ConfigureAwait(false);
    ...
    public virtual async Task ListResourcesAsync()
    {
        ...
        this._resources = new EquatableList<TResource>(await (await this.ApiClient.ManageNamespaced<TResource>().ListAsync("all").ConfigureAwait(false)).ToListAsync().ConfigureAwait(false));
        this.Reduce(s => s with
        {
    ...
    public virtual async Task DeleteResourceAsync(TResource resource)
    {
        ...
        var match = this._resources?.ToList().FirstOrDefault(r => r.GetName() == resource.GetName() && r.GetNamespace() == resource.GetNamespace());
        var resourceCollectionChanged = false;
        if (match != null)
        {
            this._resources!.Remove(match);
