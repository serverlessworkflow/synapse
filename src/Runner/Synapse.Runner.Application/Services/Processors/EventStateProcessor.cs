...
namespace Synapse.Runner.Application.Services.Processors
{
        ...
        : StateProcessor<EventStateDefinition>
    {
        ...
        {
            //TODO: remove
            this.Logger.LogInformation("TRIGGER RESULT");

            List<V1WorkflowActivity> childActivies = (await this.ExecutionContext.ListActiveChildActivitiesAsync(this.Activity, cancellationToken))
            ...
            if (this.State.Exclusive
                || childActivies.Count == this.State.Triggers.Count)
            {
                JObject output = new();
                foreach (V1WorkflowActivity childPointer in childActivies
        ...
        /// <param name="cancellationToken">A <see cref="CancellationToken"/></param>
        /// <returns>A new awaitable <see cref="Task"/></returns>
        protected virtual async Task OnTriggerCompletedAsync(IWorkflowActivityProcessor processor, CancellationToken cancellationToken)
        {
            ...
            if (this.State.Exclusive
                || childActivities
                    .Where(p => p.Type == V1WorkflowActivityType.EventStateTrigger)
