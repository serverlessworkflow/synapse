...
namespace Synapse.Runner.Application.Services.Processors
{
        ...
        : WorkflowActivityProcessor
    {
        ...
        public EventStateTriggerProcessor(ILoggerFactory loggerFactory, IWorkflowExecutionContext executionContext, IWorkflowActivityProcessorFactory activityProcessorFactory, V1WorkflowActivity activity, EventStateDefinition state, EventStateTriggerDefinition trigger)
        ...
        protected override async Task InitializeAsync(CancellationToken cancellationToken)
        {
            if (this.Activity.Status <= V1WorkflowActivityStatus.Initializing)
            {
                ...
                {
                    foreach (string eventReference in trigger.Events)
                    {
